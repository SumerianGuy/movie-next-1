...
import getGenreTvShows from 'actions/getGenreTvShows';
...
const Genre = () => {
  ...
  const [selectedMediaType] = useState(Cookies.get('selectedMediaType') || 'movie');
  ...
  useEffect(() => {
    (async () => {
      ...
      if (selectedMediaType === 'tv') {
        dispatch(getGenreTvShows(genreId, page, sortByOptionValue));
  ...
  return (
    <PageWrapper>
      <PaddingWrapper>
        <Head>
          <title>{`${general.selectedMenuItemName} ${selectedMediaType}`}</title>
        </Head>
        <Header
          title={general.selectedMenuItemName}
          subtitle= {selectedMediaType} />
        <SortBy
