    ...
    <LazyLoad
      ...
      <PosterLink
        ...
        <Scenery
          ...
          src={`${baseUrl}w${W342H513.WIDTH}${tv.poster_path}`} />
        <DetailsPanelWrapper theme={theme}>
        <PosterTitle
          theme={theme}
          className={POSTER_TITLE_CLASS_NAME}>
          {tv.name && tv.name.trim() !== '' ? tv.name : tv.original_name}
        </PosterTitle>
          <RatingInfo
            className={RATING_INFO_CLASS_NAME}
            voteAverage={tv.vote_average}
            tooltip={`${tv.vote_average} average rating on ${tv.vote_count} votes`} />
        </DetailsPanelWrapper>
      </PosterLink>
    </LazyLoad>
    <style jsx>{`
